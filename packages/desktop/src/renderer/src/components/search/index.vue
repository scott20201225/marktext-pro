...
const lastSearchOptions = reactive({
...
const rememberSearchContext = () => {
  if (!searchValue.value) return
  lastSearchValue.value = searchValue.value
  lastSearchOptions.isCaseSensitive = isCaseSensitive.value
  lastSearchOptions.isWholeWord = isWholeWord.value
  lastSearchOptions.isRegexp = isRegexp.value
}

const restoreLastSearchContext = () => {
  if (!lastSearchValue.value) return false
  searchValue.value = lastSearchValue.value
  isCaseSensitive.value = lastSearchOptions.isCaseSensitive
  isWholeWord.value = lastSearchOptions.isWholeWord
  isRegexp.value = lastSearchOptions.isRegexp
  return true
}

const ensureSearchReady = () => {
  if (highlightCount.value > 0) return true
  if (!searchValue.value && !restoreLastSearchContext()) return false
  searchFn()
  return true
}
