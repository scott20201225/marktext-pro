  ...
  return withTrampolineToken(async token => {
    ...
    try {
      return await fn({
        ...
        GIT_ASKPASS: '',
        PATH: getTrampolinePathEnvironment(customEnv),
        ...
        GIT_CONFIG_PARAMETERS: `${gitEnvConfigPrefix}'credential.helper=' 'credential.helper=desktop' 'http.version=HTTP/1.1'`,
...
export function getDesktopAskpassTrampolinePath(): string {
  return Path.resolve(getDesktopTrampolineDirectory(), getDesktopAskpassTrampolineFilename())
}
...
function getTrampolinePathEnvironment(
  ...
  return currentPath
    ? `${getDesktopTrampolineDirectory()}${Path.delimiter}${currentPath}`
    : getDesktopTrampolineDirectory()
}
