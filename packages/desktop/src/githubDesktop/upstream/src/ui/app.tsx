...
export class App extends React.Component<IAppProps, IAppState> {
  ...
  private renderAppMenuBar() {
    // MarkTextPro embeds GitHub Desktop in a BrowserView and places the menu in
    // its own left rail. Standalone upstream still keeps the original platform rule.
    if (!__WIN32__ && !window.__MARKTEXTPRO_EMBEDDED__) {
      return null
    }
  ...
  private renderTitlebar() {
    if (window.__MARKTEXTPRO_EMBEDDED__) {
  ...
  private onConfirmRepositorySwitch = async (
    ...
    if (switchWorkspace && repository instanceof Repository) {
      ipcRenderer.send(
